...
const products = [
  { name: 'Mitutoyo SJ-210', category: 'Surface Roughness Tester', code: '178-560-01D', image: '/images/sj210.jpg' },
  { name: 'Mitutoyo Crysta-Apex S', category: 'Coordinate Measuring Machine', code: 'CMM-CRYSTA-APEX', image: '/images/cmm.jpg' },
  { name: 'Mitutoyo Quick Vision', category: 'Vision Measuring System', code: 'QV-APEX-404', image: '/images/quick-vision.jpg' },
]
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><div className="flex items-end justify-between gap-6"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.22em] text-primary">Featured systems</p><h2 className="mt-4 max-w-xl font-mono text-3xl font-bold uppercase tracking-[-0.04em] sm:text-5xl">Tools for a tighter tolerance.</h2></div><Link href="/products" className="hidden text-xs font-bold uppercase tracking-widest underline decoration-primary underline-offset-8 sm:block">View all products <ArrowRight className="ml-2 inline size-4" /></Link></div><div className="mt-12 grid gap-px bg-border md:grid-cols-3">{products.map((product) => <Link href={`/products/${product.name.toLowerCase().replaceAll(' ','-')}`} key={product.name} className="group bg-background p-5"><div className="flex aspect-[4/3] items-center justify-center bg-secondary"><div className="relative size-36 border-8 border-foreground/80 transition duration-500 group-hover:scale-105"><div className="absolute -right-10 top-1/2 h-5 w-16 -translate-y-1/2 bg-primary" /><div className="absolute -bottom-5 left-7 h-12 w-8 bg-foreground/80" /></div></div><div className="flex items-start justify-between py-5"><div><p className="font-mono text-[10px] font-bold uppercase tracking-widest text-primary">{product.category}</p><h3 className="mt-2 font-mono text-lg font-bold uppercase">{product.name}</h3><p className="mt-2 text-xs text-muted-foreground">{product.code}</p></div><ChevronRight className="mt-1 size-5 transition group-hover:translate-x-1" /></div></Link>)}</div></section>
